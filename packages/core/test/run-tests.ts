...
run("packs four 50x50 pieces into one 100x100 sheet", () => {
...
run("adds another sheet when one sheet is not enough", () => {
  const plan = solveCuttingPlan({
    stock: { width: 100, height: 100 },
...
run("respects edge margin and kerf", () => {
  const plan = solveCuttingPlan({
...
run("marks pieces as unplaced when they cannot fit even on an empty sheet", () => {
  const plan = solveCuttingPlan({
    stock: { width: 100, height: 100 },
...
run("respects per-sheet rotation settings", () => {
